...
/**
 * NEW: Regenerate post from scratch (single platform only)
 */
export async function regeneratePost(postId) {
    ...
    const briefId = post.brief_id;
    const platformId = post.platform_id;

    // Delete existing post and versions
    ...
    try {
        ...
        console.log(`✅ Regenerated post for ${platform.display_name}`);
        return newPost;
    } catch (error) {
        console.error(`❌ Failed to regenerate for ${platform.display_name}:`, error.message);
        throw error;
    }
}
